...
func TestGetLastAllocatedPageID(t *testing.T) {
	dm, cleanup := setupFileDiskManager(t)
	defer cleanup()
	...
	if dm.GetLastAllocatedPageID() != 0 {
		t.Fatalf("expected last allocated page ID to be 0, got %d", dm.GetLastAllocatedPageID())
	}
}
